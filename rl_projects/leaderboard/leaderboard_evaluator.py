...
def get_weather_id(weather_conditions):
    ...
    weather_xml = 'leaderboard/data/weather.xml'
    if not os.path.exists(weather_xml):
        weather_xml = 'data/weather.xml'
...
class LeaderboardEvaluator(object):
    ...
    def _setup_simulation(self, args):
        ...
        cmd1 = [os.path.join(self.carla_path, 'CarlaUE4.sh')]
        if args.carla_render_mode == 'offscreen':
            cmd1.append('-RenderOffScreen')
        if not args.carla_sound:
            cmd1.append('-nosound')
        if args.carla_quality_level:
            cmd1.append(f'-quality-level={args.carla_quality_level}')
        cmd1.extend([f'-carla-rpc-port={args.port}', f'-graphicsadapter={args.gpu_rank}'])
        if args.carla_extra_args:
            cmd1.extend(shlex.split(args.carla_extra_args))
        self.server = subprocess.Popen(cmd1, preexec_fn=os.setsid)
        print(' '.join(cmd1), self.server.returncode, flush=True)
...
def main():
    ...
    parser.add_argument('--carla-render-mode', choices=['offscreen', 'windowed'],
                        default=os.environ.get('CARLA_RENDER_MODE', 'offscreen'),
                        help='CARLA render mode. offscreen adds -RenderOffScreen; windowed opens the normal CARLA window.')
    parser.add_argument('--carla-sound', action='store_true',
                        default=os.environ.get('CARLA_SOUND', '0').lower() in ('1', 'true', 'yes', 'on'),
                        help='Enable CARLA sound. By default the evaluator starts CARLA with -nosound.')
    parser.add_argument('--carla-quality-level', default=os.environ.get('CARLA_QUALITY_LEVEL', 'Low'),
                        help='CARLA quality level passed as -quality-level=<value>. Use empty string to omit it.')
    parser.add_argument('--carla-extra-args', default=os.environ.get('CARLA_EXTRA_ARGS', ''),
                        help='Extra arguments appended to the CARLA launch command.')
